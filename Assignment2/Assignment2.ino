...
const byte PIN_ANALOG_X = 0; //พอร์ตที่ใช้เชื่อมต่อ
...
void Pause_game(){ //หน้า pause เกมส์
//  lcd.fillScreen(BLACK);
...
//    limit_delete_hp_enemy = true;
  ...
  if(hp_enemy <= 3){//สร้างยานอวกาศศัตรู
...
///เช็คกระสุนศัตรูว่าโดนยานของเราหรือไม่
...
//           Serial.println("Collision");
    ...
    }else{// stage ที่ศัตรูยิงกระสุนทีละ 2 ครั้ง
...
//      }
   ...
   ///จบศัตรู///
    ...
    ////ยานของเรา////
  if(changepo_ship_x <= 10 && ship_x>2){//เคลื่อนที่ไปทางซ้าย
      pre_chip_x = ship_x;
...
//      Serial.println(changepo_ship_x);
   ...
   if(changepo_ship_x >= 1000 && ship_x<116){//เคลื่อนที่ไปทางขวา
      pre_chip_x = ship_x;
      if(state_bullet == false){//ปรับความเร็วยาน  เวลายิงกระสุนแล้วเคลื่อนช้า
        ship_x += 1;
        }
...
//      Serial.println(changepo_ship_x);
    ...
    if(button_c == 0 && state_bullet != false){//ยานเรายิงกระสุน
      state_bullet = false;
...
//           Serial.println("Collision");
  ...
  if(hp <= 0 || hp_enemy <= 0){//เช็คค่าเลือด
    over_game = true;
    lcd.fillScreen(BLACK);
...
void random_enemy(){//สุ่มเคลื่อนที่ซ้าย-ขวาศัตรู
    if(count_move == 0){  
...
int random_shot_enemy(){//สุ่มว่าศัตรูจะยิงหรือไม่
    return random(0,2);
  }

void bullet_enemy_coliision_ship(int x,int y,int pre_y,int index){//สุ่มว่ากระสุนเฟส 2 ยิงโดนยานเรามั้ย
  hitbox[0] = ship_x;
  hitbox[1] = ship_x + ship_width;
  hitbox[2] = ship_y;
...
//            Serial.println("Collision");
 ...
 void random_item(){//สุ่มเกิดไอเทม
